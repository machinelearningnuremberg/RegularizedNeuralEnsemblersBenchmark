...
class MTNeuralEnsembler(BaseEnsembler):
    ...
    def update_context_size(self, context_size: int = None):
    ...
    def get_batch_for_pretraining(self, meta_split="meta-train"):
        dataset_name = np.random.choice(
            self.metadataset.meta_splits[meta_split], 1
        ).item()
        ...
        if self.predefined_pipeline_ids is not None:
            ...
            predefined_pipeline_ids = np.random.randint(
                0, self.num_pipelines, self.metadataset.get_num_pipelines()
            )

        # X and base functions are the same
        X = base_functions = (
            self.metadataset.get_predictions([predefined_pipeline_ids])[0]
            .transpose(0, 1)
            .transpose(2, 1)
            .unsqueeze(0)
        )[:, samples_idx]
        ...
        return self.send_to_device(
            X, base_functions, y, X_context, y_context, mask_context
        )
    ...
    def fit_net(
        self, X_train, y_train, base_functions_train, dropout_rate: float | None = None
    ):
        self.training = True
    ...
    def validate(self, model, X_val, y_val, base_functions_val):
        X_val, y_val, base_functions_val = self.send_to_device(
            X_val, y_val, base_functions_val
        )
    ...
    def meta_validate(self, model, validation_iterations=100):
        ...
        for i in range(validation_iterations):
            ...
            with torch.no_grad():
                ...
                logits = self.metadataset.get_logits_from_probabilities(output)
                loss = self.criterion(
                    logits.reshape(-1, num_classes), y_batch.reshape(-1)
                ).item()
                val_loss += loss
...
class EFFNet(nn.Module):  # Sample as Sequence
    ...
    def forward(
        ...
        for i in range(0, num_classes, self.inner_batch_size):
            ...
            temp_w = self.get_batched_weights(
                x=x[:, :, range_idx],
                base_functions=base_functions[:, :, range_idx],
                y=y,
