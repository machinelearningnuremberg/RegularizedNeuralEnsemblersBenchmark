...
DEFAULT_MODEL_ARGS = {
    "classification": {"svm": {"probability": True}, "catboost": {"iterations": 100}}
}
...
class ScikitLearnStacker(BaseEnsembler):
        ...
        **kwargs,
        ...
        if self.model_args is None:
            if self.task_type in DEFAULT_MODEL_ARGS.keys():
                self.model_args = DEFAULT_MODEL_ARGS[self.task_type].get(
                    self.model_name, {}
                )
            else:
                self.model_args = {}
    ...
    def sample(self, X_obs, **kwargs):
        ...
        base_functions = (
            self.metadataset.get_predictions([X_obs])[0].transpose(0, 1).transpose(2, 1)
        ).numpy()
