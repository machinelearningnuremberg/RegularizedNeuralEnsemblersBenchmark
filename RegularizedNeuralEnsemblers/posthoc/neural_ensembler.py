    ...
    def auto_dropout_and_fit(
        ...
        for dropout_rate in dropout_rate_list:
            ...
            for train_idx, val_idx in kf.split(idx):
                net = self.fit_net(
                    X_train[:, train_idx], y_train[:, train_idx], dropout_rate
                )
                temp_val_loss = self.validate(
                    net, X_train[:, val_idx], y_train[:, val_idx]
                )
    ...
    def forward(self, x):
        ...
        for i in range(0, num_classes, self.inner_batch_size):
            ...
            if self.add_y:
                ...
                z = torch.repeat_interleave(z.unsqueeze(1), temp_x.shape[1], dim=1)
