...
class CMAESEnsembler(BaseEnsembler):
    ...
    def get_weights(self, *args, **kwargs):
        ...
        weights = torch.FloatTensor(self.cmaes.weights_).unsqueeze(
            0
        )  # change when omitting the batch size
    ...
    def sample(
        ...
        **kwargs,
        ...
        for model_id in X_obs:
            model_prediction = self.metadataset.get_predictions([[model_id]])[0][
                0
            ].numpy()
            predictions.append(model_prediction)

        labels = self.metadataset.get_targets().numpy()
        self.cmaes.ensemble_fit(predictions=predictions, labels=labels)
        ...
        (
            ...
            metric_per_pipeline,
            _,
        ) = self.metadataset.evaluate_ensembles_with_weights([X_obs], weights)
