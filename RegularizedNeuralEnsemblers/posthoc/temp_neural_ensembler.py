...
class NeuralEnsembler(BaseEnsembler):
    ...
    def prune_weights(self, weights):
        ...
        weight_pruner = nn.Threshold(weight_thd, 0.0)
    ...
    def sample(self, X_obs, **kwargs) -> tuple[list, float]:
        ...
        if self.auto_dropout:
            self.net = self.auto_dropout_and_fit(X_train=base_functions, y_train=y)
        else:
            self.net = self.fit_net(X_train=base_functions, y_train=y)
    ...
    def auto_dropout_and_fit(
        self, X_train, y_train, num_folds=3, dropout_rate_list=[0, 0.25, 0.5, 0.75]
    ):
        num_samples, num_classes, num_base_functions = X_train.shape
        idx = np.arange(num_samples)
        np.random.shuffle(idx)
        ...
        for dropout_rate in dropout_rate_list:
            ...
            for train_idx, val_idx in kf.split(idx):
                net = self.fit_net(X_train[train_idx], y_train[train_idx], dropout_rate)
                temp_val_loss = self.validate(net, X_train[val_idx], y_train[:, val_idx])
    ...
    def fit_net(self, X_train, y_train, dropout_rate: float | None = None):
        self.training = True
        ...
        X_train, y_train, model = self.send_to_device(X_train, y_train, model)
...
class ENetSimple(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.dropout_rate > 0 and self.training:
            mask = (
                (torch.rand(size=(num_base_functions,)) > self.dropout_rate)
                .float()
                .to(x.device)
            )
            for i, dim in enumerate([num_samples, num_classes]):
                mask = torch.repeat_interleave(mask.unsqueeze(i), dim, dim=i)
        ...
        w_norm = w_norm.reshape(num_samples, num_classes, num_base_functions)
...
class EFFNet(nn.Module):  # Sample as Sequence
    ...
    def get_mask_and_scaling_factor(self, num_base_functions, device):
        ...
        scaling_factor = 1.0 / (1.0 - dropout_rate)
    ...
    def forward(self, x):
        ...
        if self.dropout_is_active and self.training:
            mask, scaling_factor = self.get_mask_and_scaling_factor(
                num_base_functions, x.device
            )
            for i, dim in enumerate([num_samples, num_classes]):
                mask = torch.repeat_interleave(mask.unsqueeze(i), dim, dim=i)
        else:
        ...
        for i in range(0, num_classes, self.inner_batch_size):
            ...
            if mask is not None:
                ...
                base_functions[:, range_idx] = (
                    base_functions[:, range_idx] * mask[:, range_idx]
                )
            else:
                temp_x = x[:, range_idx]
            ...
            if self.add_y:
                ...
                z = self.class_embedding(torch.LongTensor(range_idx).to(temp_x.device))
                z = torch.repeat_interleave(z.unsqueeze(0), temp_x.shape[0], dim=0)
        ...
        if self.mode == "model_averaging":
            ...
            w = torch.repeat_interleave(w.unsqueeze(1), num_classes, dim=1)
