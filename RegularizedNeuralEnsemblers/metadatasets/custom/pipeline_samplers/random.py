...
class RandomPipelineSampler(BasePipelineSampler):
    def __init__(
        self, num_pipelines: int = 20, random_state: int = 42, task_type="classification"
    ):
    ...
    def sample(self):
        ...
        for i in range(self.num_pipelines):
            ...
            model_class = copy.deepcopy(
                list(self.param_distributions.keys())[model_class_id]
            )
            parameters = list(
                ParameterSampler(
                    param_distributions=self.param_distributions[model_class],
                    n_iter=1,
