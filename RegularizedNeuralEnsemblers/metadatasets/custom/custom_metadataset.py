...
class CustomMetaDataset(Evaluator):
    ...
    def set_state(
        ...
        if base_pipelines is None:
            ...
            if self.base_pipelines is None:
                self.base_pipelines, self.hp_candidates = RandomPipelineSampler(
                    self.num_base_pipelines,
    ...
    def precompute_predictions(self):
        ...
        for pipeline in self.base_pipelines:
            if self.task_type == "classification":
                predictions.append(
                    torch.FloatTensor(pipeline.predict_proba(self.X_val)).unsqueeze(0)
                )
            elif self.task_type == "regression":
                predictions.append(
                    torch.FloatTensor(pipeline.predict(self.X_val)).unsqueeze(0)
                )
            else:
        ...
        if self.task_type == "classification":
            self.predictions = torch.nan_to_num(
                self.predictions, nan=1.0 / self.num_classes, posinf=1.0, neginf=0.0
            )
