...
class Evaluator(BaseMetaDataset):
    def __init__(
        ...
        self.absolute_relative_error = lambda y_true, y_pred: torch.abs(
            y_true - y_pred
        ) / torch.max(torch.ones(1), torch.abs(y_true))
    ...
    def subsample(self, predictions, targets):
        if self.split in ["valid", "val"] and self.pct_valid_data < 1.0:
            valid_data_size = max(
                int(self.pct_valid_data * predictions.shape[2]), predictions.shape[3]
            )
            idx = np.random.default_rng(seed=self.seed).integers(
                0, predictions.shape[2], valid_data_size
            )
    ...
    def _compute_metrics(
        ...
        if self.metric_name == "error":
            ...
            metric_per_sample = self.absolute_relative_error(
                temp_targets, predictions.squeeze(-1)
            )
            metric_per_pipeline = metric_per_sample.mean(-1)
            metric_ensemble_per_sample = self.absolute_relative_error(
                temp_targets, weighted_predictions.sum(axis=1, keepdim=True).squeeze(-1)
            )
            ...
            metric_per_pipeline = metric_per_sample.mean(-1)
            metric_ensemble_per_sample = self.mse(
                temp_targets, weighted_predictions.sum(axis=1, keepdim=True).squeeze(-1)
            )
            metric = metric_ensemble_per_sample.reshape(batch_size, -1).mean(-1)
            ...
            for i in range(y_pred.shape[0]):
                ...
                metric.append(
                    1
                    - roc_auc_score(
                        targets[i].numpy(), y_pred_.numpy(), multi_class="ovo"
                    )
                )
