...
class NASBench201MetaDataset(Evaluator):
    ...
    def get_features(self, ensembles: list[list[int]]) -> torch.Tensor:
        ...
        all_model_ids = {id for sublist in ensembles for id in sublist}

        # Query all needed model_ids at once
        all_needed_features = self._features[
            self._features["model_id"].isin(all_model_ids)
        ].compute()
        ...
        for model_ids in ensembles:
            ...
            for model_id in model_ids:
                matched_rows = all_needed_features[
                    all_needed_features["model_id"] == model_id
                ]
                ensemble_features.append(matched_rows)
            ensemble_features_df = pd.concat(ensemble_features, ignore_index=True)
            temp_features_tensor = torch.tensor(
                ensemble_features_df.drop(columns="model_id").values, dtype=torch.float32
            )
