...
import pandas as pd
import torch
from tabrepo import EvaluationRepository, get_context, list_contexts, load_repository

from ..base_metadataset import BaseMetaDataset
from ..evaluator import Evaluator

DATA_VERSION_TO_CONTEXT = {
    "version0": "D244_F3_C1530_3",
...
DATA_VERSION_TO_TASK_TYPE = {"class": "classification", "reg": "regression"}

TASK_TYPE_TO_METADATASET = {
...
class TabRepoMetaDataset(Evaluator):
    ...
    def set_state(self, dataset_name: str, split: str = "valid", fold: int = 0):
        self.split = split
        self.fold = fold
        self.dataset_metadata = self.repo.dataset_metadata(dataset_name)
        super().set_state(dataset_name=dataset_name, split=split or self.split)
    ...
    def impute_candidates(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df[categorical_cols] = df[categorical_cols].apply(
            lambda col: col.fillna("nan").astype(str)
        )
    ...
    def _posprocess_binary_predictions(self, predictions: np.array) -> np.array:
        if self.dataset_metadata["NumberOfClasses"] == 2:
            predictions = torch.cat(
                [1 - predictions.unsqueeze(-1), predictions.unsqueeze(-1)], axis=-1
            )
    ...
    def get_predictions(self, ensembles: list[list[int]]) -> torch.Tensor:
        ...
        for configs in config_names_in_ensemble:
            if self.split == "valid":
                temp_predictions = torch.FloatTensor(
                    self.repo.predict_val_multi(
                        dataset=self.dataset_name, fold=self.fold, configs=configs
                    )
                )

            else:
                temp_predictions = torch.FloatTensor(
                    self.repo.predict_test_multi(
                        dataset=self.dataset_name, fold=self.fold, configs=configs
                    )
                )
            if self.task_type == "classification":
        ...
        predictions = torch.nan_to_num(
            predictions, nan=1.0 / num_classes, posinf=1.0, neginf=0.0
        )
        return predictions
    ...
    def score_ensemble(self, ensemble: list[int]):
        score, _ = self.repo.evaluate_ensemble(
