...
from .ftc_args import ftc_args, ftcplus_args, ftctest_args, mini_ftc_args
from .hub import DATASETS, MODELS
...
class FTCMetaDataset(Evaluator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.files = ["times.csv", "test_predictions.csv", "val_predictions.csv"]
    ...
    def _load_predictions(self, config_id):
        ...
        if path.exists():
            return [
                torch.FloatTensor(pd.read_csv(path / file, index_col=0).values)
    ...
    def _preprocess_hps(self, hps):
        hps = pd.get_dummies(pd.DataFrame(hps)).fillna(0).astype(float).values
    ...
    def _load_data(self, dataset_name=None):
        ...
        for (config_id, config), bash_args in zip(args_dict.items(), args_list):
            ...
            if data is not None:
                ...
                if (
                    test_predictions.isnan().sum() > 0
                    or val_predictions.isnan().sum() > 0
                ):
                    ...
                    self.val_predictions[dataset_name].append(
                        val_predictions.unsqueeze(0)
                    )
                    self.test_predictions[dataset_name].append(
                        test_predictions.unsqueeze(0)
                    )
        ...
        for dataset_name in self.val_predictions.keys():
            self.val_targets[dataset_name] = torch.cat(
                self.val_predictions[dataset_name]
            )[0, :, -1].long()
            self.test_targets[dataset_name] = torch.cat(
                self.test_predictions[dataset_name]
            )[0, :, -1].long()
            self.val_predictions[dataset_name] = torch.cat(
                self.val_predictions[dataset_name]
            )[..., :-1]
            self.test_predictions[dataset_name] = torch.cat(
                self.test_predictions[dataset_name]
            )[..., :-1]
            ...
            self.all_hp_candidates[dataset_name] = self._preprocess_hps(
                self.row_hp_candidates[dataset_name]
            )
            self.all_hp_candidates_ids[dataset_name] = torch.arange(
                len(self.row_hp_candidates[dataset_name])
            )
    ...
    def _get_hp_candidates_and_indices(self) -> tuple[torch.Tensor, torch.Tensor]:
        return (
            self.all_hp_candidates[self.dataset_name],
            self.all_hp_candidates_ids[self.dataset_name],
        )
