...
from ..metadatasets.ftc.hub import MODELS
from ..metadatasets.ftc.metadataset import FTCMetaDataset
from ..posthoc.single_best import SingleBest

data_dir = "data"
data_version = "mini"
# data_version = "extended"
metadataset = FTCMetaDataset(metric_name="error", data_version=data_version)
...
for dataset_name in dataset_names:
    ...
    incumbent_ensemble, incumbent = posthoc_ensembler.sample(ids.tolist())
    for model in MODELS:
    ...
    metadataset.set_state(dataset_name=dataset_name, split="test")

    for model in MODELS:
        X_obs = ids[hps_df.model == model]

        if len(X_obs) > 0:
            best_val_model_id = X_obs[
                results_per_dataset_val[dataset_name][model]["best_val_model_id"]
            ]
            _, metric, metric_per_pipeline, _ = metadataset.evaluate_ensembles(
                [[best_val_model_id]]
            )
            metric_per_pipeline = metric_per_pipeline.numpy()
