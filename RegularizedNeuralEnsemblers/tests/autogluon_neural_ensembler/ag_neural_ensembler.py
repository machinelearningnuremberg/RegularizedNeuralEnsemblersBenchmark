...
class AutoGluonNeuralEnsembler(AbstractModel):
    ...
    def _preprocess(self, X: pd.DataFrame, is_train=False, **kwargs) -> list[np.ndarray]:
        ...
        if is_train:
            self._oof_prediction_features = self.feature_metadata.get_features(
                required_special_types=["stack"]
            )

            if self.problem_type == MULTICLASS:
                self._base_models = list(
                    {x.rsplit("_", 1)[0] for x in self._oof_prediction_features}
                )
        ...
        for bm in self._base_models:
            if self.problem_type == BINARY:
                ...
                all_f_for_bm = [
                    f for f in self._oof_prediction_features if f.startswith(f"{bm}_")
                ]
                assert (
    ...
    def _predict_proba(self, X, **kwargs) -> np.ndarray:
        ...
        y_pred = (
            self.model.predict(X).cpu().detach().numpy()
        )  # does not have a concept of predict proba
