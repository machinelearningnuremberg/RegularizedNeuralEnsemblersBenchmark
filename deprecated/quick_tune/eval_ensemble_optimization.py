...
class EnsembleOptimization:
    ...
    def _initialize_ensemble(self, how: str = "randomly"):
        ...
        if how == "randomly":
            self.ensemble = np.random.randint(
                0, self.num_candidates, self.ensemble_size
            ).tolist()
    ...
    def _get_observations(self):
        ...
        ensemble_ids = (
            np.array(self.ensemble)
            .reshape(1, -1)
            .repeat(len(self.observed_pipeline_ids), 0)
        )
    ...
    def _get_next_pipeline_to_add(self) -> Tuple[int, float]:
        ...
        if self.use_oracle:
            return (
                self.metadataset.get_best_performance_idx().item(),
                self.metadataset.get_best_performance().item(),
            )
        else:
            ensemble_ids = (
                np.array(self.ensemble)
            ...
            observed_ids = np.array(self.observed_pipeline_ids).reshape(-1, 1)
            ids = np.concatenate((ensemble_ids, observed_ids), axis=1)
            pipeline_hps, y, _ = self.metadataset.get_y_ensemble(
                ids, batch_size=ids.shape[0]
            )
            y = y.cpu().numpy()
    ...
    def post_hoc_ensembling(self, initial_ensemble, ensemble_size=5):
        ...
        post_hoc_regret_test = self.test_best_ensemble_on_dataset(
            ensemble=post_hoc_ensemble
        )
    ...
    def random_ensemble(self, ensemble_size=5, replace=True):
        ensemble = np.random.choice(
            self.observed_pipeline_ids, ensemble_size, replace=replace
        ).tolist()
    ...
    def optimize(self):
        ...
        for i in range(self.num_iterations):
            ...
            print(
                "Iteration: {}, Best performance: {}".format(
                    i, np.min(val_performance_curve)
                )
            )
    ...
    def save_results(
        self, val_curve, test_curve, time_curve, best_ensemble, experiment_id
    ):
        results_dict = {
            "val_perf": val_curve,
        ...
        save_path = os.path.join(
            current_dir, "..", "results", experiment_id, self.dataset_name
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        choices=["", "oracle", "post_hoc", "random"],
    )
    ...
    for dataset in metadataset.datasets:
        ...
        if optimization_type == "post_hoc":
            (
                ...
                runtime_post,
                best_ensemble_post,
            ) = eo.post_hoc_ensembling(eo.best_ensemble, ensemble_size=ensemble_size)
            eo.save_results(
                val_perf_post,
                test_perf_post,
                runtime_post,
            ...
            (
                ...
                test_perf_random,
                runtime_random,
                best_ensemble_random,
            ) = eo.random_ensemble(ensemble_size=ensemble_size)
            eo.save_results(
                val_perf_random,
                test_perf_random,
                runtime_random,
                best_ensemble_random,
                experiment_id + "_random",
            )
