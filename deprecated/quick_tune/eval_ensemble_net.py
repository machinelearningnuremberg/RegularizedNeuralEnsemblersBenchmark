...
class TransferBO4E:
    ...
    def candidates_generator_v1(
        self, iter=2, subset_size=5, permute=True
    ) -> List[List[int]]:
        if len(self.observed_pipeline_ids) > 2:
            for i in range(self.generator_iter):
                if len(self.observed_pipeline_ids) > self.subset_size:
                    subset_of_pipeline_ids = np.random.choice(
                        self.observed_pipeline_ids, self.subset_size, replace=False
                    )
                else:
                    subset_of_pipeline_ids = self.observed_pipeline_ids

                n = np.random.randint(
                    1, min(len(subset_of_pipeline_ids), self.max_ensemble_size)
                )

                if self.permute:
                    precandidates = list(
                        itertools.permutations(subset_of_pipeline_ids, n)
                    )
                else:
                    precandidates = list(
                        itertools.combinations(subset_of_pipeline_ids, n)
                    )
                ...
                if self.state == "greedy":
                    ...
                    for precandidate in precandidates[:1000]:
                        ...
                        subset_of_single_candidates = np.arange(
                            len(self.single_candidates)
                        )
                        subset_of_single_candidates = np.random.choice(
                            subset_of_single_candidates, self.subset_size, replace=False
                        )
    ...
    def candidates_generator_v2(
        self, iter=2, subset_size=15, permute=True
    ) -> List[List[int]]:
        ...
        for _ in range(iter):
            if len(self.observed_pipeline_ids) > 2:
                if len(self.observed_pipeline_ids) > subset_size:
                    subset_of_pipeline_ids = np.random.choice(
                        self.observed_pipeline_ids, subset_size, replace=False
                    )
                else:
                    subset_of_pipeline_ids = self.observed_pipeline_ids

                n = np.random.randint(
                    1, min(self.max_ensemble_size, len(subset_of_pipeline_ids))
                )
                if permute:
                    precandidates = list(
                        itertools.permutations(subset_of_pipeline_ids, n)
                ...
                if not self.state:
                    ...
                    for k in range(len(precandidates)):
                        ...
                        subset_of_single_candidates = np.random.choice(
                            subset_of_single_candidates, subset_size, replace=False
                        )
            ...
            for elite_member in self.ensemble_elite + [[]]:
                if len(elite_member) > 1:
                    ...
                    candidates2 = []
                    n = np.random.randint(
                        1, min(self.max_ensemble_size, len(elite_member))
                    )
                    ...
                    if (
                        len(candidates1) > 0 and self.state
                    ):  # and len(candidates1[0]) > 1:
                        # dont yield single candidates that are know
                        print(len(candidates1))
                        yield candidates1

                    if (
                        len(candidates2) > 0 and not self.state
                    ):  # and len(candidates2[0]) > 1
    ...
    def acqf(self, candidates: List[List[int]]) -> torch.FloatTensor:
        ...
        for i in range(1, self.model.num_encoders):
            if len(self.observed_pipeline_ids) == 0:
                ...
                x_support.append(
                    self.hp_candidates[idxs].unsqueeze(0).repeat(len(candidates), 1, 1)
                )
                yp_support.append(
                    self.observed_response_per_pipeline[idxs]
                    .unsqueeze(0)
    ...
    def observe(self, ensemble: List[int]):
        ...
        (
            observed_pipelines_in_ensemble,
            unobserved_pipelines_id,
            index_in_ensemble,
        ) = self.get_unobserved_pipelines(ensemble.tolist())
        ...
        for i, pipeline_id in enumerate(unobserved_pipelines_id):
            ...
            total_time += (
                self.metadataset.time_info[["train_time", self.dataset_split + "_time"]]
                .iloc[pipeline_id]
                .sum()
            )
        ...
        if (
            self.retrain
            and (len(self.observed_pipeline_ids) > self.min_observed_pipelines_to_retrain)
    ...
    def suggest(self) -> List[int]:
        ...
        for candidates in self.candidates_generator():
            if self.state == "explore":
                ...
                _, y_values, _ = self.metadataset.get_y_ensemble(
                    candidates, batch_size=len(candidates)
                )
    ...
    def get_normalized_regret(self) -> float:
        ...
        normalized_regret = (
            best_perf - np.array(self.observed_response_per_ensemble).max()
        ) / (best_perf - worst_perf)
    ...
    def init_for_dataset(self, dataset_name, split="val"):
        ...
        self.hp_candidates = torch.FloatTensor(
            self.metadataset.get_hp_candidates().values
        ).to(self.device)
        self.num_candidates = len(self.hp_candidates)
        self.single_candidates = torch.arange(self.num_candidates).reshape(-1, 1).tolist()

        # reset variables
        self.observed_response_per_pipeline = torch.zeros(self.num_candidates).to(
            self.device
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="max number of pipelines in the ensemble during learning",
    )
    ...
    for split_id in split_ids:
        ...
        metadataset = EnsembleMetaDataset(
            batch_size=batch_size,
            data_version=data_version,
            max_num_pipelines=max_num_pipelines,
            split_id=split_id,
            data_path=os.path.join(
                current_dir, "..", "..", "AutoFinetune", "aft_data", "predictions"
            ),
            seed=seed,
        )
        # x = loader.get_batch()
