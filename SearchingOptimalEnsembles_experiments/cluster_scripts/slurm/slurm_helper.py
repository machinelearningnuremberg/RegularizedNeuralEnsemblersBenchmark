...
CPU_PARTITIONS = [
    ...
    "mlstudents_cpu-ivy",
    "ml_cpu-ivy",
    "bosch_cpu-cascadelake",
    "testbosch_cpu-cascadelake",
    "gki_cpu-cascadelake",
]

parser = ArgumentParser()
parser.add_argument(
    "jobfile", metavar="Job file", nargs=1, help="A file with one job per line"
)
parser.add_argument(
    "--startup",
...
parser.add_argument(
    "--timelimit", type=int, help="Timelimit of job in seconds", required=True
)
...
group.add_argument(
    ...
    "--queue",
    type=str,
    choices=CPU_PARTITIONS + GPU_PARTITIONS,
...
parser.add_argument(
    "--hold", default=False, help="Submit job in hold state", action="store_true"
)
...
parser.add_argument(
    "-o", "--output", default=".", help="Output directory of the generated .pbs file"
)
parser.add_argument(
    "-l", "--logfiles", default=".", help="Output directory of the log files of the job"
)
parser.add_argument(
    "--max_running_tasks",
    default=None,
...
parser.add_argument(
    "--memory_per_job", dest="memory", default="4000mb", help="Memory limit per job"
)
...
array_job = f"{start_array_jobs_at}-{end_array_jobs_at}"
if args.max_running_tasks is not None:
    array_job += f"%{args.max_running_tasks}"
...
output_file = os.path.join(args.output, f"{jobfile_filename}_{time_string}.pbs")
...
if not args.local:
    ...
    submission_prolog = r"""{ { id | egrep "\(aad\)|\(aad-hiwi\)" > /dev/null; } || { echo ""; echo "Error: You are not a member of the AAD-Group"; echo ""; exit 1; } } 2>&1
    ...
    if not args.dry_run:
        ...
        stderr_queue = Queue()  # type: ignore
        stdout_queue = Queue()  # type: ignore
