...
if __name__ == "__main__":
    ...
    for file in files:
        try:
            ...
            time_info = {"train_time": train_time, "test_time": test_time, "val_time": val_time}
            ...
            if "predictions" not in aggregated_info[dataset_name].keys():
                ...
                aggregated_info[dataset_name]["predictions"] = np.concatenate((aggregated_info[dataset_name]["predictions"], predictions), axis=0)
    ...
    for dataset_name in aggregated_info.keys():
        ...
        aggregated_info[dataset_name]["predictions"] = aggregated_info[dataset_name]["predictions"].astype(np.float16)
        np.save(os.path.join(save_path, "per_dataset", dataset_name , "predictions.npy"), aggregated_info[dataset_name]["predictions"])
        aggregated_info[dataset_name]["targets"] = aggregated_info[dataset_name]["targets"].astype(np.int)
        np.save(os.path.join(save_path, "per_dataset", dataset_name , "targets.npy"), aggregated_info[dataset_name]["targets"])
        aggregated_info[dataset_name]["split_indicator"] = aggregated_info[dataset_name]["split_indicator"].reshape(-1).astype(int)
        np.save(os.path.join(save_path, "per_dataset", dataset_name , "split_indicator.npy"), aggregated_info[dataset_name]["split_indicator"])
        ...
        with open(os.path.join(save_path, "per_dataset", dataset_name ,"args.json"), "w") as f:
            ujson.dump(aggregated_info[dataset_name]["args"], f)
        with open(os.path.join(save_path, "per_dataset", dataset_name ,"time_info.json"), "w") as f:
