...
def train(model, loader,
    ...
    for i in range(num_epochs):
        ...
        if i%val_freq == 0 and observed_pipeline_ids is None :
            val_loss, k_coef = test(model, loader, criterion, mode="val", num_epochs=num_epochs_test)
...
def ECE(model, loader, num_bins=50):
    ...
    with torch.no_grad():
        x, y, y_per_pipeline = loader.get_batch(num_encoders=model.num_encoders, batch_size=1000)
        ...
        for i in range(1,num_bins):
            if np.sum(bin_indices==i)>0:
                ...
                rmse[i-1] = np.sqrt(np.mean(np.power(true_rank[bin_indices==i] - mean[bin_indices==i],2)))
