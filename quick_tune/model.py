...
class EnsembleLoader:
    ...
    def get_batch(self, mode="train",
        ...
        if return_context:
            context = self.get_context(dataset, mode=mode, max_context_size = self.max_context_size)
            ...
            context = torch.cat([pipeline_hps_context.squeeze(1), metric_context.unsqueeze(-1)], axis=-1)
...
class EnsembleNet(nn.Module):
    def __init__(self, dim_in, hidden_dim=64, hidden_dim_ff=32, num_heads=4,
                    num_seeds=1, out_dim=32, out_dim_ff=1, return_context=False):
        super().__init__()
        ...
        if return_context:
            self.hidden_layer = nn.Linear(in_features=out_dim*2, out_features=hidden_dim_ff)
        else:
...
def test(model, loader, criterion, mode="val", return_context=False, num_epochs = 200):
    ...
    for _ in range(num_epochs):
        with torch.no_grad():
            if return_context:
                x, y, context, acc_per_pipeline = loader.get_batch(mode=mode, return_context=True)
                y_pred = model(x, context, acc_per_pipeline)
            else:
