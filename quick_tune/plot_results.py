...
def read_results(results_path, dataset_names):
    ...
    for dataset_name in dataset_names:
        ...
        #read json file
        try:
            with open(os.path.join(results_path, dataset_name, "results.json"), "r") as f:
...
def gather_results(experiments, results_path):
    ...
    for experiment_id in experiments:
        try:
            temp_results = read_results(os.path.join(results_path, experiment_id), dataset_names)
        except Exception as e:
            print(e)
            continue

        unorganized_results["aggregated_val_perf"][experiment_id] = temp_results["aggregated_val_perf"]
        unorganized_results["aggregated_test_perf"][experiment_id] = temp_results["aggregated_test_perf"]
    ...
    for dataset in dataset_names:
        try:
            ...
            temp_results_test = []
            for experiment_id in experiments:
                temp_results_val.append(unorganized_results["aggregated_val_perf"][experiment_id][dataset])
                temp_results_test.append(unorganized_results["aggregated_test_perf"][experiment_id][dataset])
            results["aggregated_val_perf"].append(temp_results_val)
            results["aggregated_test_perf"].append(temp_results_test)
        except Exception as e:
            print("Error in dataset: ", dataset, " , experiment:", experiment_id, " , error: ", e)
            continue
    return results
...
def plot_results(experiment_ids, experiment_names, add_confidence_interval=False,
                                                    save_path=None):
    ...
    all_results = [(regret_val_mean, regret_val_std), (regret_test_mean, regret_test_std),
                   (val_results_rank_mean, val_results_std_rank), (test_results_rank_mean, test_results_std_rank)]
    ...
    for i, (means, stds) in enumerate(all_results):
        ...
        for j in range(len(experiment_names)):
            ...
            if add_confidence_interval:
                axs[i1, i2].fill_between(np.arange(len(mean)), mean - std, mean + std, alpha=0.2)
...
experiment_names = ["EO", "MetaEnsembleLearning", "BO", "Oracle", "NoMetaLearned1", "NoMetaLearned2", "T1", "T2"]
