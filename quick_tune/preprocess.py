...
NON_CAT_COLS = [ 'batch_size', 'bss_reg', 'cotuning_reg', 'cutmix', 'decay_epochs', 'decay_rate', 'delta_reg',
                'drop', 'linear_probing', 'lr', 'mixup', 'mixup_prob', 'momentum', 'patience_epochs',
                'pct_to_freeze', 'ra_magnitude', 'ra_num_ops', 'random_augment', 'smoothing', 'sp_reg', 'stoch_norm',
                'trivial_augment', 'warmup_epochs', 'warmup_lr', 'weight_decay', 'clip_grad', 'layer_decay']

def preprocess_args(args_df, drop_constant_args=False, encode_categorical_args=True, impute_numerical_args=True):
    index = args_df.index
...
KEEP_COLUMNS = ["batch_size", "bss_reg", "cotuning_reg", "cutmix", "random_augment", "delta_reg", "drop", "lr", "mixup",
                "mixup_prob", "ra_num_ops", "ra_magnitude", "patience_epochs", "opt_betas", "weight_decay", "warmup_lr",
                "warmup_epochs", "stoch_norm", "opt", "sched", "trivial_augment", "layer_decay", "auto_augment",
                "linear_probing", "clip_grad", "smoothing", "pct_to_freeze", "sp_reg", "model", "momentum", "decay_epochs",
                "decay_rate", "dataset"
                ]
...
if __name__ == "__main__":
    ...
    with open(aggregated_args_path, "r") as f:
        args = json.load(f)
    ...
    preprocessed_df_args.to_csv("../AutoFinetune/aft_data/predictions/preprocessed_args.csv", index=False)