...
import RegularizedNeuralEnsembler.metadatasets.scikit_learn.metadataset as slmd
import torch

try:
    import RegularizedNeuralEnsembler.metadatasets.tabrepo.metadataset as trmd
except ImportError:
    trmd = None
import matplotlib.cm as cm
import RegularizedNeuralEnsembler.metadatasets.nasbench201.metadataset as nbmd
...
def get_weights_and_scores(dataset_name, dropout_rate):
    ...
    model_metrics = (model_metrics - model_metrics.min()) / (
        model_metrics.max() - model_metrics.min()
    )
    ensemble_metric = (ensemble_metric.item() - model_metrics.min()) / (
        model_metrics.max() - model_metrics.min()
    )
...
for i, dropout_rate in enumerate(dropout_rates):
    ...
    for dataset_name in dataset_names:
        weights, model_metrics, ensemble_metric = get_weights_and_scores(
            dataset_name, dropout_rate
        )
...
)  # Assuming integer values are between 1 and 5
cmap = cm.viridis
for i, dropout_rate in enumerate(data.keys()):
    for dataset_name in data[dropout_rate].keys():
        ...
        axs2[i].scatter(
            weights,
            np.ones(len(model_metrics)) * ensemble_metric,
            alpha=0.2,
            color="blue",
            s=600,
        )
