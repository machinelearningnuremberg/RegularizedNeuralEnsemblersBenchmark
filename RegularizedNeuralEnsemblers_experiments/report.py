...
class Reporter:
    def __init__(
        ...
        if project_configs_path is None:
            project_configs_path = (
                current_path / "cluster_scripts" / "slurm_pineda" / "experiments_confs"
            )
        ...
        if self.plot_by_groups:
            ...
            for group in self.report_structure["experiment_groups"].keys():
                self.experiment_names.extend(
                    list(self.report_structure["experiment_groups"][group].keys())
                )
                self.experiment_table_names.extend(
                    list(self.report_structure["experiment_groups"][group].values())
                )
                self.groups.extend(
                    [group]
                    * len(self.report_structure["experiment_groups"][group].keys())
                )
            self.translator_to_table_names = dict(
                zip(self.experiment_names, self.experiment_table_names)
            )
        else:
            self.experiment_names = self.report_structure["experiment_names"].keys()

    def load_project_config(self, project_name):
        with open(self.project_configs_path / (project_name + ".yml")) as file:
        ...
        for project_name in self.report_structure["project_names"].keys():
            ...
            for experiment_name in self.experiment_names:
                for dataset_id in range(num_datasets):
                    if dataset_id in self.dataset_filter:
                        continue

                    for seed in range(num_seeds):
                        temp_path = (
                            self.experiments_results_path
                            / project_name
                            / experiment_name
                            / str(dataset_id)
                            / str(seed)
                        ...
                        try:
                            ...
                            missing_experiments_list[project_name][
                                experiment_name
                            ].append(dataset_id)
                            passed = False
    ...
    def write_missing_experiments(self, missing_experiments_list):
        ...
        for project_name, experiments in missing_experiments_list.items():
            for experiment_name, missing_experiments in experiments.items():
                missing_experiments_counts[project_name][experiment_name] = len(
                    missing_experiments
                )
        ...
        with open(
            self.report_output_path / "missing_experiments_list.yml", "w"
        ) as yaml_file:
            yaml.dump(missing_experiments_list, yaml_file)

        with open(
            self.report_output_path / "missing_experiments_counts.yml", "w"
        ) as yaml_file:
    ...
    def process_table(self, original_table):
        table = pd.pivot_table(
            original_table,
            values=["val_metric", "test_metric"],
            index="experiment_name",
            columns=["project_name", "dataset_id", "seed"],
        )
        runtimes = pd.pivot_table(
            original_table,
            values=["posthoc_total_time"],
            index="experiment_name",
            columns=["project_name", "dataset_id", "seed"],
        )
        ...
        if self.baseline is not None:
            for column in table.columns:
                ...
                if not self.per_dataset:
                    ...
                    table[column][
                        table[column] > 10
                    ] = 10  # imputing the very large errors
    ...
    def write_tables(self, tables):
        def highlight_smallest(m, s, smallest, second_smallest):
            return (
                "\\textbf{" + str(m) + r"}$_{\pm" + str(s) + "}$"
                if m == smallest
                else (
                    "\\underline{\\textit{" + str(m) + r"}}$_{\pm" + str(s) + "}$"
                    if m == second_smallest
                    else str(m) + r"$_{\pm" + str(s) + "}$"
                )
            )
        ...
        for table_name, table_group in tables.items():
            ...
            # table.columns.names=["",""]
            smallest_ones = table_mean.apply(lambda x: x.min()).apply(
                lambda x: f"{x:,.4f}"
            )
            second_smallest_ones = table_mean.apply(
            ...
            for column in table_mean.columns:
                table[column] = table[column].apply(lambda x: f"{x:,.4f}")
                table[column + "_std"] = table[column + "_std"].apply(
                    lambda x: f"{x:,.4f}"
                )
                ...
                if not self.per_dataset:
                    table[column] = table[[column, column + "_std"]].apply(
                        lambda x: highlight_smallest(
                            x[column], x[column + "_std"], smallest, second_smallest
                        ),
                        axis=1,
                    )
    ...
    def make_plot_by_groups(self, df, fontsize=30):
        for project in self.report_structure["project_names"]:
            ...
            bars = plt.bar(
                df["name"], df[project], color=df["color"], width=0.9, zorder=1
            )
            ...
            plt.grid(
                True, which="both", axis="y", linestyle="--", linewidth=0.7, zorder=0
            )  # Grid behind bars with zorder=0

            legend_handles = [
                plt.Rectangle((0, 0), 1, 1, color=color_map[group]) for group in groups
            ]
    ...
    def make_plot_as_curves(self, table, fontsize=30, linewidth=5):
        plt.figure(figsize=(10, 10))
        plt.grid(
            True, which="both", axis="y", linestyle="--", linewidth=0.7, zorder=-1
        )  # Grid behind bars with zorder=0
        plt.grid(
            True, which="both", axis="x", linestyle="--", linewidth=0.7, zorder=-1
        )  # Grid behind bars with zorder=0

        for project_name in table.columns:
            plt.plot(table[project_name], label=project_name, linewidth=linewidth)

        plt.axhline(y=1.0, color="black", linestyle="--", linewidth=linewidth * 0.5)
    ...
    def report(self):
        ...
        if self.plot_by_groups:
            ...
            self.write_tables(
                {
                    "aggregated": {"mean": aggregated_mean, "std": aggregated_std},
                    "ranked": {
                        "mean": ranked_mean,
                        "std": ranked_std,
                    },
    ...
    def make_double_plot_as_curves(self, table_paths, titles, fontsize=24, linewidth=5):
        ...
        for i, table_path in enumerate(table_paths):
            ...
            for project_name in table.columns[1:]:
                axs[i].plot(
                    values, table[project_name], label=project_name, linewidth=linewidth
                )

            axs[i].axhline(
                y=1.0, color="black", linestyle="--", linewidth=linewidth * 0.5
            )
        ...
        fig.savefig(
            self.report_output_path / (f"double_curves_plot.png"), bbox_inches="tight"
        )
        fig.savefig(
            self.report_output_path / (f"double_curves_plot.pdf"), bbox_inches="tight"
        )
