...
class QuicktuneMetaDataset(BaseMetaDataset):
    def __init__(
        ...
        self.hps = pd.read_csv(os.path.join(self.data_dir, "preprocessed_args.csv"))
    ...
    def get_statistics_dataset(self, dataset_name):

        hp_candidates, time_info, predictions, targets = self.get_dataset_info(dataset_name)
        pipeline_ids = np.arange(len(hp_candidates)).reshape(-1,1).tolist()
    ...
    def get_dataset_info(self, dataset_name: str = "",):
        ...
        self.targets = np.array(self.aggregated_info[dataset_name]["targets"])
        self.predictions = np.array(self.aggregated_info[dataset_name]["predictions"])
        ...
        self.targets = torch.unsqueeze(torch.LongTensor(self.targets), 0)
        ...
        return self.pipeline_hps, self.time_info, self.predictions, self.targets
    ...
    def set_dataset (self, dataset_name: str):
        ...
        if dataset_name != self.dataset_name:
            ...
            self.get_statistics_dataset(dataset_name)
    ...
    def evaluate_ensembles(
        ...
        if self.metric_name == "acc":
            metric_per_pipeline = (predictions.argmax(axis=-1) == targets.unsqueeze(1)).float().mean(axis=-1)
            metric_per_sample = (predictions.reshape(-1, n_classes).argmax(axis=1) == targets.reshape(-1)).float()
            metric = metric_per_sample.reshape(batch_size, -1).mean(axis=1)
        elif self.metric_name == "nll":
            metric_per_pipeline = None
            metric_per_sample = cross_entropy(predictions.reshape(-1, n_classes), targets.reshape(-1))
            metric = metric_per_sample.reshape(batch_size, -1).mean(axis=1)
        else:
