...
class DivBO(BaseSearcher):
    ...
    def __init__(
        ...
        **kwargs, # pylint: disable=unused-argument
        ...
        self.posthoc_ensembler = GreedyEnsembler(metadataset=self.metadataset, device=self.device)
        self.performance_surrogate_name = surrogate_name
        self.diversity_surrogate_name = diversity_surrogate_name

        self.performance_surrogate = self._build_surrogate(surrogate_name, **surrogate_args)
        self.diversity_surrogate = self._build_surrogate(diversity_surrogate_name, **diversity_surrogate_args)
    ...
    def diversity_acquisition(self, X_pool, X_pending):
        ...
        for i in range(len(X_pool)):
            temp_X = np.concatenate([np.tile(X_pool[i], (len(X_pending), 1)), X_pending], axis=1)
            mean, std = self.diversity_surrogate.predict(temp_X)
            X = np.random.normal(mean.reshape(1, -1), std.reshape(1, -1), (self.num_samples, len(X_pending)))
            div_matrix[i] = X
    ...
    def suggest(
        ...
        **kwargs, # pylint: disable=unused-argument
        ...
        if len(self.X_obs) == 0:
            ...
            x1, x2, score = self.diversity_sampler.sample_with_diversity_score(observed_pipeline_ids=self.X_obs)
            x1 = x1.cpu().numpy()
            x2 = x2.cpu().numpy()
            ...
            if len(self.X_obs) > self.pool_size:
                pool, _ = self.posthoc_ensembler.sample(X_obs=self.X_obs,
                                                 max_num_pipelines=self.pool_size)

            else:
                pool = self.X_obs

            #acquisition function
            pipeline_hps, _, _, _  = self.metadataset.evaluate_ensembles(ensembles=[self.X_pending])
